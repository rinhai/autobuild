...
# 后台IP设置
export Ipv4_ipaddr="192.168.22.252"            # 修改openwrt后台地址(填0为关闭)
...
# 旁路由选项
export Gateway_Settings="192.168.22.1"                 # 旁路由设置 IPv4 网关(填入您的网关IP为启用)(填0为不作修改)
export DNS_Settings="192.168.22.1"                     # 旁路由设置 DNS(填入DNS，多个DNS要用空格分开)(填0为不作修改)
...
sed -i 's/"TTYD 终端"/"命令"/g' `egrep "TTYD 终端" -rl ./`
sed -i 's/"USB 打印服务器"/"打印服务"/g' `egrep "USB 打印服务器" -rl ./`
sed -i 's/"Web 管理"/"Web管理"/g' `egrep "Web 管理" -rl ./`
sed -i 's/"管理权"/"修改密码"/g' `egrep "管理权" -rl ./`
